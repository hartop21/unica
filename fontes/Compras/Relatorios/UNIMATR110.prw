...
/*/
User Function UniMatr110( cAlias, nReg, nOpcx )
...
Static Function ReportDef(nReg,nOpcx)
	...
	Local oSection2
	Local nTamDscPrd 	:= 0
	/* Inicio de customizacao */
	Local nTamPrdForn	:= 0
	/* Final de customizacao */
	...
	If Upper(AllTrim(mv_par06)) $ "B1_DESC|B5_CEME|C7_DESCRI"
		nTamDscPrd := TamSX3(AllTrim(mv_par06))[1] + 5
	Else
		nTamDscPrd := TamSX3("B1_DESC")[1] + 5
	EndIf
	nTamPrdForn    := TamSX3("A5_CODPRF")[1]
	...
	/* Inicio de Customizacao */
	TRCell():New(oSection2, "PRODFORN"   	, "   ", "Cod.Prod.Forn.",/*Picture*/, nTamPrdForn,/*lPixel*/, {|| cProdXForn},,,,,, .F.)
	/* Final de customizacao */
...
Static Function ReportPrint(oReport,nReg,nOpcX)
	...
	While !oReport:Cancel() .And. !SC7->(Eof()) .And. SC7->C7_FILIAL == cFilSC7 .And. SC7->C7_NUM >= mv_par01 .And. SC7->C7_NUM <= mv_par02
		...
		For nVias := 1 to mv_par09
			...
			oReport:SkipLine()
			oReport:SkipLine()
			oReport:PrintText('Caro Fornecedor',, 050)
			oReport:SkipLine()
			oReport:PrintText('Por favor verifique se todas as condições comerciais acordadas estão corretas, Preço, Prazo de pagamento, VPC, Frete, Custos, referência interna etc.',, 050)
			oReport:PrintText('Caso algo esteja em desacordo, retorne para o comprador para dirimir e corrigir quaisquer dúvidas e emissão de um novo PDF com as condições corretas.',, 050)
			oReport:PrintText('Não havendo qualquer contato do fornecedor em relação ao PDF enviado, entenderemos que o pedido está correto e a NF deverá ter as mesmas condições explicitadas no pedido.',, 050)
			oReport:PrintText('Se a NF estiver em divergente do pedido, a mercadoria será devolvida no ato do recebimento.',, 050)
			oReport:PrintText('Só aceitaremos a mercadoria se na sua Nota Fiscal constar o número do nosso Pedido de Compras.',, 050)
			oReport:SkipLine()
			oReport:PrintText('Obrigado',, 050)
			oReport:SkipLine()
			/* Final de customizacao */
			...
			// oReport:PrintText( STR0011+SubStr(SE4->E4_CODIGO,1,40)+ " - " + SubStr(SE4->E4_DESCRI,1,34),nLinPC,050 )
			/* Inicio de customizacao */
			...
			cObs01 := STR0011+SubStr(SE4->E4_CODIGO,1,40)+ " - " + SubStr(SE4->E4_DESCRI,1,34)

			cxCondPag := Alltrim(GetSX3Cache("A2_XTPFRET","X3_TITULO")) + ": "
			aCBox := StrToKArr(GetSX3Cache("A2_XTPFRET","X3_CBOX"),";")
			cPesq := SA2->A2_XTPFRET + "="
			cxCondPag += If((nEle := AScan(aCBox, {|x| cPesq $ x})) > 0, StrTran(aCBox[nEle],cPesq,""), "")
			// oReport:PrintText( cxCondPag,nLinPC, 050 )
			cObs02 := cxCondPag
			cObs02 += " - " + Alltrim(GetSX3Cache("A2_XVPC","X3_TITULO")) + ": " + AllTrim(SA2->A2_XVPC)

			cxCondPag := AllTrim(GetSX3Cache("A2_XPVPC","X3_TITULO")) + ": "
			aCBox := StrToKArr(GetSX3Cache("A2_XPVPC","X3_CBOX"),";")
			cPesq := SA2->A2_XPVPC + "="
			cxCondPag += If((nEle := AScan(aCBox, {|x| cPesq $ x})) > 0, StrTran(aCBox[nEle],cPesq,""), "")

			cObs03 := cxCondPag

			cxCondPag := AllTrim(GetSX3Cache("A2_XPRZPAG","X3_TITULO")) + ": "
			aCBox := StrToKArr(GetSX3Cache("A2_XPRZPAG","X3_CBOX"),";")
			cPesq := SA2->A2_XPRZPAG + "="
			cxCondPag += If((nEle := AScan(aCBox, {|x| cPesq $ x})) > 0, StrTran(aCBox[nEle],cPesq,""), "")
...
Static Function CabecPCxAE(oReport,oSection1,nVias,nPagina)
	...
	nLinPC := oReport:Row()
	oReport:PrintText(STR0124 + Transform(SM0->M0_CGC,cPicA2_CGC) ,nLinPC,15)
	If cPaisLoc == "BRA"
