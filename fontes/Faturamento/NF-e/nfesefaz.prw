...
User Function XmlNfeSef(cTipo,cSerie,cNota,cClieFor,cLoja,cNotaOri,cSerieOri)
	...
	Local lEIPIOutro 	:= GetNewPar("MV_EIPIOUT",.F.) .And. lEipiDev //Quando habilitado juntamente com o MVIPIDEV, o valor do IPI será gerado na tag vOutro, destacado nas informações complementares e no campo OUTRAS DESPESAS ACESSORIAS.
						...
						#IFDEF TOP
							...
							BeginSql Alias cAliasSE1
								...
								FROM
									%Table:SE1% SE1
								WHERE
									SE1.E1_FILIAL = %Exp:cFilTit%
									AND SE1.E1_PREFIXO = %Exp:cPrfTit%
									AND SE1.E1_NUM = %Exp:cNumDupl%
									AND (
										(SE1.E1_TIPO = %Exp:MVNOTAFIS%)
										OR (SE1.E1_TIPO = 'DP ')
										OR (
											(SE1.E1_ORIGEM IN ('LOJA701', 'FATA701', 'LOJA010'))
											AND SE1.E1_TIPO IN (%Exp:cWhere%)
										)
									)
									AND SE1.%NotDel%
								ORDER BY
...
Static Function NfeItem(aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aAnfI,cTipo,cVerAmb,aComb,cMensFis,cCsosn,aPedCom,aNota,aICMSZFM,aDest,cIpiCst,aFCI,lIcmDevol,nVicmsDeson,nVIcmDif,cMunPres,aAgrPis,aAgrCofins,nIcmsDif,aICMUFDest,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,cAmbiente,aIPIDevol,nvBCUFDest, aItemVinc,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aLote,cMensDifal, aTotICMSST, nTotProd, nItProd, nValDifer)
	...
	If Len(aICMSZFM) > 0 .And. Len(aCST) > 0 .And. !Empty(aICMSZFM[1])
		...
		Endif
	Else
		//Adicionado por Luiz Otavio para corrigir rejeicao 610 * em 09/09/2021
		cString += NfeTag('<vDesc>' ,ConvType((aProd[15]),15,2))
	EndIf
	...
	if len (cSeek)>0
		...
		BeginSql Alias cAliasSFT
			...
			WHERE
				...
				AND SFT.%NotDel%
				AND FT_NFISCAL = %Exp:cNfiscal%
				AND FT_SERIE = %Exp:cSerie%
				AND FT_TIPOMOV = %Exp:"E"%
				AND FT_CLIEFOR = %Exp:cCliFor%
				AND FT_LOJA = %Exp:cLoja%
				AND FT_ITEM = %Exp:SD1->D1_ITEM%
				AND FT_PRODUTO = %Exp:cProduto%
		EndSql
...
Static Function AjustaDest(aDest,aNfVinc,cCliefor,cLoja)
	...
	BeginSql Alias cAliasAIF
		...
		SELECT
			max(AIF_DATA) AIF_DATA
		FROM
			%Table:AIF% AIF
		WHERE
			AIF.AIF_FILIAL = %xFilial:AIF%
			AND AIF.AIF_FILTAB = %Exp:xFilial("SA1")%
			AND AIF.AIF_TABELA = %Exp:"SA1"%
			AND AIF.AIF_CODIGO = %Exp:cCliefor%
			AND AIF.AIF_LOJA = %Exp:cLoja%
			AND AIF.AIF_CAMPO IN (
				'A1_NOME',
				'A1_END',
				'A1_COMPLEM',
				'A1_BAIRRO',
			...
			AND AIF.AIF_DATA >= %Exp:dDataEmis%
			AND AIF.%NotDel%
	EndSql
